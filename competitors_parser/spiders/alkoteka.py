...
class AlkotekaSpider(BaseCompetitorSpider):
    ...
    BASE_API_CITY_UUID = '?city_uuid=4a70f9e0-46ae-11e7-83ff-00155d026416' # Краснодар
    BASE_API_ITEM_ON_PAGE = '&page=1&per_page=20&'     # 1стр, 20 на стр
    BASE_API_URL_CAT = (
        f'{BASE_API_URL_BEGIN}{BASE_API_CITY_UUID}{BASE_API_ITEM_ON_PAGE}'
        'root_category_slug='
    )
    BY_STOCKS = False
    ...
    def parse(self, response: Response) -> Iterator[Request]:
        ...
        category_name = title.replace('купить в г.Краснодар  по доступной цене', '')
        category_list_url = [url.replace(self.BASE_URL_CAT, self.BASE_API_URL_CAT) for url in self.start_urls]
...
#            print(products)
            ...
            for product in products:
                ...
                url = f'{self.BASE_API_URL_BEGIN}/{product_slug}{self.BASE_API_CITY_UUID}'

                yield Request(
                    url=url,
                    callback=self.parse_product,
                    cb_kwargs={
                        'cat': f'{cat} --- {product_title}',
                        'slug': f'{product_slug}'
                    },
                    dont_filter=False
                )
    ...
    def parse_product(
        ...
        try:
            ...
            product_url = f'{self.BASE_URL_PROD}{subcategory_slug}/{slug}'    # нужен еще слаг подкатегории
